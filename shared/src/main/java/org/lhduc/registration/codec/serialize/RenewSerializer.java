...
public class RenewSerializer implements PacketSerializer<RenewPacket> {
    ...
    @Override
    public void write(OutputStream out, RenewPacket packet) throws IOException {
    ...
    @Override
    public RenewPacket read(InputStream in, PacketHeader header) throws IOException {
        DataInputStream dis = (DataInputStream) in;
        String clientId = dis.readUTF();
        UUID sessionId = new UUID(dis.readLong(), dis.readLong());
        return new RenewPacket(header, clientId, sessionId);
    }
}
